...
def fill_order(order, txes=[]):
    ...
    unfilled_db = g.session.query(Order).filter(Order.filled == None).filter(Order.sell_currency==order.buy_currency).filter((Order.sell_amount / Order.buy_amount) >= (order.buy_amount/order.sell_amount)).all()
                ...
                if (existing_order.sell_amount / existing_order.buy_amount) >= (order.buy_amount/order.sell_amount) :
                    if is_valid(existing_order):
                        ...
                        if (existing_order.buy_amount > order.sell_amount) | (order.buy_amount > existing_order.sell_amount) :
                            ...
                            child['sell_amount'] = 1+int((parent.buy_amount-counter.sell_amount)*(parent.sell_amount/parent.buy_amount))  
                            ...
                            #print(tx_parent['platform'],tx_parent['amount'],file=sys.stderr)
                            ...
                            #print(tx_counter['platform'],tx_counter['amount'],file=sys.stderr)
